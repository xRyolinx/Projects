...
int main(void)
{
    while(1)
    {
        ...
        FILE* nb = fopen("nombre.txt", "r");
        int number;
        fread(&number , sizeof(int) , 1 , nb);

        //Reading students file
        FILE* std = fopen("eleves.txt", "r");
        ...
        if (result == 0)
        {
            ...
            if (inscri == 1)
            {
                ...
                fwrite(new , 1 , endf , stud);

                printf("%s a ete inscrit !\n", eleve);
        ...
        if (rep[0] == 'R' && inscri != 0)
        {
            ...
            //Modify names
            FILE* addname = fopen("eleves.txt" , "w");
            for (int i = 0 ; i < number ; i++)
            {
...
void modify(char* reader , FILE* modi)
{
    ...
    if (strcmp(rep , "t") == 0)
    {
        ...
        if (check1 == -1)
        {
            printf("Voulez-vous vraiment changer le type 'groupe' en 'individuel' ?\n");
        ...
        if (check2 == -1)
        {
            printf("Voulez-vous vraiment changer le type 'individuel' en 'groupe' ?\n");
            ...
            else
            {
                ...
                printf("Tappez le chiffre correspendant : ");
                char* grp = malloc(2);
                scanf("%s", grp);
